                                                                                        ...
                                                                                        <div class="dropdown-menu dropdown-menu-right">
                                                                                            ...
                                                                                            <!--  Start Paystack Payment -->
                                                                                                ...
                                                                                                @if(!empty($is_paystack) && $balance_amount !=0)
                                                                                                    <form method="POST" action="{{ route('pay-with-paystack') }}" accept-charset="UTF-8" class="form-horizontal" role="form">
                                                                                                        ...
                                                                                                        <input type="hidden" name="amount" value="{{$fees_assigned->fees_amount * 100}}">
                                                                                                        <input type="hidden" name="quantity" value="1">
                                                                                                        <input type="hidden" name="fees_type_id" value="{{$fees_assigned->feesGroupMaster->fees_type_id}}">
                                                                                                        <input type="hidden" name="student_id" value="{{$student->id}}">
                                                                                                        <input type="hidden" name="reference" value="{{ Paystack::genTranxRef() }}">
                                                                                                        <input type="hidden" name="record_id" value="{{$fees_assigned->record_id}}">
                                                                                            ...
                                                                                            <!-- Start Khalti Payment -->
                                                                                                @if ( (moduleStatusCheck('KhaltiPayment') == TRUE)  && ($balance_amount > 0) )
                                                                                                    ...
                                                                                                    @endphp
                                                                                                    <div class="pay">
                                                                                                        <button type="button" class="dropdown-item btn filled small khalti-payment-button" data-amount="{{$balance_amount}}" data-assignid="{{$fees_assigned->id}}" data-feestypeid="{{$fees_assigned->feesGroupMaster->fees_type_id}}", data-recordId = "{{@$fees_assigned->recordDetail->id}}">
                                                                                            ...
                                                                                            <!-- Start Raudhahpay Payment  -->
                                                                                                ...
                                                                                                @if( (moduleStatusCheck('Raudhahpay') == TRUE) && ($balance_amount != 0) and $is_active )
                                                                                                    <form action="{!!route('raudhahpay.feesPayment')!!}" method="POST" style="width: 100%; text-align: center">
                                                                                                        @csrf
                                                                                            ...
                                                                                            <!-- Start Paypal Payment -->
                                                                                                ...
                                                                                                @if(!empty($is_paypal) && $balance_amount !=0)
                                                                                                    <form method="POST" action="{{ route('payByPaypal') }}" accept-charset="UTF-8" class="form-horizontal" role="form">
                                                                                                        @csrf
                                                                                                        <input type="hidden" name="assign_id" value="{{$fees_assigned->id}}">
                                                                                                        <input type="hidden" name="url" id="url" value="{{URL::to('/')}}">
            ...
            @if($createdBy != "")
                ...
                @if(!empty( $created_by ))
                    <a href="#" data-toggle="tooltip" data-placement="right"
                        title="{{'Collected By: '.$created_by->full_name}}">@lang('fees.discount_of') {{generalSetting()->currency_symbol}}{{ number_format($fees_discount->feesDiscount->amount, 2, '.', '') }} @lang('fees.applied')
                        : {{$createdBy->id.'/'.$createdBy->created_by}}</a>
                @endif
            @endif
        @else
            @lang('fees.discount_of') {{generalSetting()->currency_symbol}}{{$fees_discount->feesDiscount !=""?number_format($fees_discount->feesDiscount->amount, 2, '.', '') :""}} @lang('fees.assigned')
        @endif
    </td>

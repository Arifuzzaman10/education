...
if (!function_exists('send_mail')) {
    function send_mail($reciver_email, $receiver_name, $purpose, $data = [])
    {
        ...
        $body = SmsTemplate::emailTempleteToBody(getTempleteDetails($purpose)->body, $data);
...
if (!function_exists('studentRecords')) {
    ...
    {
        $studentRecord = StudentRecord::query()->with('classes', 'studentDetail');
        if ($student_id != null) {
...
if (!function_exists('SchoolModuleStatus')) {
    function SchoolModuleStatus($schoolmodule)
    {
        ...
        $school_modules = Cache::rememberForever('school_module_' . $school_id, function () use ($school_id) {
            return SchoolModule::where('school_id', $school_id)
                ->where('active_status', 1)
                ->get();
        });


        return $school_modules->contains('module_name', strtolower($schoolmodule));
    }
}
...
if(!function_exists('teacherParentService')) {
    function teacherParentService()
    {
        return false;
    }
}
